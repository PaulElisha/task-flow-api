...
class WorkspaceController {
    ...
    createWorkspace = async (req, res) => {
        ...
        const error = WorkspaceService.validate({ userId: req.user._id, body: req.body });
        ...
        try {
            ...
            res.status(200).json({ message: 'Workspace created successfully', status: 'ok', workspace });
        } catch (error) {
    ...
    getAllWorkspacePerMember = async (req, res) => {
        try {
            const { workspace } = await this.workspaceService.getAllWorkspacePerMember(req.user._id);
            res.status(200).json({ message: 'User workspaces fetched successfully', status: 'ok', workspace });
        } catch (error) {
            res.status(500).json({ message: error.message, status: 'error' })
        }
    }
    ...
    getWorkspaceById = async (req, res) => {
        ...
        try {
            ...
            res.status(200).josn({ message: 'Workspace fetched successfully', status: 'ok', workspace });
        } catch (error) {
            res.status(500).json({ message: error.message, status: 'error' })
        }
    }
    ...
    getWorkspaceMembers = async (req, res) => {
        ...
        try {
            ...
            res.status(200).json({ message: 'Workspace members retrieved', status: 'ok', data: { members, roles } })
        } catch (error) {
            res.status(500).json({ message: error.message, status: 'error' })
        }
    }
    ...
    changeMemberRole = async (req, res) => {
        ...
        try {
            ...
            res.status(200).json({ message: 'Member role changed', status: 'ok', member });
        } catch (error) {
            res.status(500).json({ message: error.message, status: 'error' })
        }
    }
    ...
    updateWorkspaceById = async (req, res) => {
        ...
        try {
            ...
            res.status(200).json({ message: 'Workspace updated', status: 'ok', workspace })
        } catch (error) {
            res.status(500).json({ message: error.message, status: 'error' });
        }
    }
